...
from .schema import Claim, ConflictLocalization, FactoidSlots
...
class FactoidDecomposer:
    ...
    def _extract_entities_llm(self, text: str) -> dict:
        ...
        Hỗ trợ cả OpenAI client object lẫn plain callable.
        """
        import json
        prompt = (
            "Extract from this claim and return as JSON:\n"
            '{"entity_subject": null, "entity_object": null, "relation": null, "location": null}\n\n'
            f"Claim: {text}\nReturn ONLY valid JSON."
        )
        try:
            client = self.client
            if callable(client) and not hasattr(client, "chat"):
                raw = str(client(prompt))
            else:
