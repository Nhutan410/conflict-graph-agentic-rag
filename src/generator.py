...
from .schema import Claim, ConflictLocalization, LoopResult
...
class AnswerGenerator:
    ...
    Args:
        ...
        openai_client: Optional injectable OpenAI client.
    """
    ...
    def _call_llm_with_retry(self, prompt: str) -> str:
        ...
        Hỗ trợ hai interface:
        - OpenAI client object: client.chat.completions.create(...)
        - Plain callable:       client(prompt) -> str
        """
        for attempt in range(self.max_retries):
            try:
                ...
                wait = 2.0 ** attempt
                logger.warning("LLM failed attempt %d/%d: %s", attempt + 1, self.max_retries, exc)
                time.sleep(wait)
        raise RuntimeError(f"Generation failed after {self.max_retries} retries")
