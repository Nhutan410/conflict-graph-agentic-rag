...
class NLIInference:
    ...
    # cross-encoder/nli-deberta-v3-base id2label: {0: contradiction, 1: entailment, 2: neutral}
    LABEL_MAP = {
        "entailment":    "support",
    ...
    def nli_pipeline(self):
        if self._pipeline is None:
            ...
            # text-classification: nhận {"text": premise, "text_pair": hypothesis}
    ...
    def predict(self, claim_a: str, claim_b: str) -> tuple[str, float]:
        ...
        try:
            result = self.nli_pipeline(
                {"text": claim_a, "text_pair": claim_b},
                truncation=True,
            )
            # result: {"label": "contradiction", "score": 0.999}
            ...
            return self.LABEL_MAP.get(top_label, "neutral"), top_score
    ...
    def predict_batch(
        ...
        """Predict relations cho nhiều pairs — batch qua NLI pipeline."""
        if not pairs:
            return []
        inputs = [{"text": a, "text_pair": b} for a, b in pairs]
        try:
            results = self.nli_pipeline(inputs, truncation=True)
            output = []
            for res in results:
                ...
                if score < self.threshold:
                    ...
                    output.append((self.LABEL_MAP.get(label, "neutral"), score))
            return output
        except Exception as exc:
            logger.warning("NLI batch prediction failed: %s. Falling back to single.", exc)
